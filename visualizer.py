...
class Visualizer:
    ...
    def prepare(self, chart_data, title):
        print('>>>visualizer.prepare')

        self.title = title
        with lock:
            print('>>>visualizer.prepare 캔버스를 초기화하고 5개의 차트를 그릴 준비')
            ...
            for ax in self.axes:
                ...
                # y axis 위치 오른쪽으로 변경
                ax.yaxis.tick_right()
            print('>>>visualizer.prepare 차트 1. 일봉 차트')
            self.axes[0].set_ylabel('Env.')  # y 축 레이블 표시
            x = np.arange(len(chart_data))
            print('>>>visualizer.prepare open, high, low, close 순서로된 2차원 배열')
            ohlc = np.hstack((x.reshape(-1, 1), np.array(chart_data)[:, 1:-1]))
            print('>>>visualizer.prepare 양봉은 빨간색으로 음봉은 파란색으로 표시')
            candlestick_ohlc(self.axes[0], ohlc, colorup='r', colordown='b')
            print('>>>visualizer.prepare 거래량 가시화')
