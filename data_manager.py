...
def preprocess(data, ver='v1'):
    ...
    if ver == 'v1.rich':
        ...
        data.loc[1:, 'inst_lastinst_ratio'] = (data['inst'][1:].values - data['inst'][:-1].values) \
        ...
        data.loc[1:, 'frgn_lastfrgn_ratio'] = (data['frgn'][1:].values - data['frgn'][:-1].values) \
...
def load_data(fpath, date_from, date_to, ver='v2'):
    ...
    print('>>>data_manager.load_data 날짜 오름차순 정렬')
    data = data.sort_values(by='date').reset_index()
    print(">>>data_manager.load_data.data=", data)

    print('>>>data_manager.load_data 데이터 전처리')
    data = preprocess(data)

    # data.to_csv("log/preprocess_data.csv", mode='w')
    # print(">>>data_manager.preprocess.data=", data)

    print('>>>data_manager.load_data 기간 필터링')
    data['date'] = data['date'].str.replace('-', '')
    data = data[(data['date'] >= date_from) & (data['date'] <= date_to)]
    data = data.dropna()

    print('>>>data_manager.load_data 차트 데이터 분리')
    chart_data = data[COLUMNS_CHART_DATA]

    print('>>>data_manager.load_data 학습 데이터 분리')
    training_data = None
    if ver == 'v1':
