...
class DNN(Network):
    ...
    def get_network_head(inp):
        print('>>>network.DNN.get_network_head')
    ...
    def train_on_batch(self, x, y):
        print('>>>network.DNN.train_on_batch')
        x = np.array(x).reshape((-1, self.input_dim))
        return super().train_on_batch(x, y)

    def predict(self, sample):
        print('>>>network.DNN.predict')
...
class LSTMNetwork(Network):
    def __init__(self, *args, num_steps=1, **kwargs):
        print('>>>network.LSTMNetwork.__init__')
        super().__init__(*args, **kwargs)
        with graph.as_default():
            if sess is not None:
                set_session(sess)
    ...
    @staticmethod
    def get_network_head(inp):
        print('>>>network.LSTMNetwork.get_network_head')
    ...
    def train_on_batch(self, x, y):
        print('>>>network.LSTMNetwork.train_on_batch')
        x = np.array(x).reshape((-1, self.num_steps, self.input_dim))
        return super().train_on_batch(x, y)

    def predict(self, sample):
        print('>>>network.LSTMNetwork.predict')
...
class CNN(Network):
    ...
    @staticmethod
    def get_network_head(inp):
        print('>>>network.CNN.get_network_head')
    ...
    def train_on_batch(self, x, y):
        print('>>>network.CNN.train_on_batch')

        x = np.array(x).reshape((-1, self.num_steps, self.input_dim, 1))
        return super().train_on_batch(x, y)

    def predict(self, sample):
        print('>>>network.CNN.predict')
        sample = np.array(sample).reshape(
