...
class ReinforcementLearner:
    ...
    def __init__(self, rl_method='rl', stock_code=None,
        ...
        print('>>>learners.ReinforcementLearner.__init__')
    ...
    def init_value_network(self, shared_network=None,
        ...
        print('>>>learners.ReinforcementLearner.init_value_network')
    ...
    def init_policy_network(self, shared_network=None,
        ...
        print('>>>learners.ReinforcementLearner.init_policy_network')
    ...
    def reset(self):
        print('>>>learners.ReinforcementLearner.reset')
    ...
    def build_sample(self):
        print('>>>learners.ReinforcementLearner.build_sample')
    ...
    def update_networks(self, batch_size, delayed_reward, discount_factor):
        print('>>>learners.ReinforcementLearner.update_networks')
    ...
    def fit(self, delayed_reward, discount_factor, full=False):
        print('>>>learners.ReinforcementLearner.fit')
    ...
    def visualize(self, epoch_str, num_epoches, epsilon):
        print('>>>learners.ReinforcementLearner.visualize')
    ...
    def run(self,
            ...
            start_epsilon=0.5,
            learning=True):
        print('>>>learners.ReinforcementLearner.run')
        ...
        print('>>>learners.ReinforcementLearner.run 시작 시간')
        time_start = time.time()

        print('>>>learners.ReinforcementLearner.run 가시화 준비')
        # 차트 데이터는 변하지 않으므로 미리 가시화
        self.visualizer.prepare(self.environment.chart_data, info)

        print('>>>learners.ReinforcementLearner.run 가시화 결과 저장할 폴더 준비')
        ...
        print('>>>learners.ReinforcementLearner.run 에이전트 초기 자본금 설정')
        self.agent.set_balance(balance)

        print('>>>learners.ReinforcementLearner.run 학습에 대한 정보 초기화')
        max_portfolio_value = 0
        epoch_win_cnt = 0

        print('>>>learners.ReinforcementLearner.run 학습 반복')
        for epoch in range(num_epoches):
            time_start_epoch = time.time()

            print('>>>learners.ReinforcementLearner.run 샘플을 만들기 위한 큐')
            q_sample = collections.deque(maxlen=self.num_steps)

            print('>>>learners.ReinforcementLearner.run 환경, 에이전트, 신경망, 가시화, 메모리 초기화')
            self.reset()

            print('>>>learners.ReinforcementLearner.run 학습을 진행할 수록 탐험 비율 감소')
            if learning:
            ...
            while True:
                print('>>>learners.ReinforcementLearner.run 샘플 생성')
                next_sample = self.build_sample()
                if next_sample is None:
                    break

                print('>>>learners.ReinforcementLearner.run num_steps만큼 샘플 저장')
                q_sample.append(next_sample)
                if len(q_sample) < self.num_steps:
                    continue

                print('>>>learners.ReinforcementLearner.run 가치, 정책 신경망 예측')
                pred_value = None
                pred_policy = None
                if self.value_network is not None:
                ...
                print('>>>learners.ReinforcementLearner.run 신경망 또는 탐험에 의한 행동 결정')
                action, confidence, exploration = self.agent.decide_action(pred_value, pred_policy, epsilon)

                print('>>>learners.ReinforcementLearner.run 결정한 행동을 수행하고 즉시 보상과 지연 보상 획득')
                immediate_reward, delayed_reward = self.agent.act(action, confidence)

                print('>>>learners.ReinforcementLearner.run 행동 및 행동에 대한 결과를 기억')
                ...
                print('>>>learners.ReinforcementLearner.run 반복에 대한 정보 갱신')
                self.batch_size += 1
                self.itr_cnt += 1
                self.exploration_cnt += 1 if exploration else 0

                print('>>>learners.ReinforcementLearner.run 지연 보상 발생된 경우 미니 배치 학습')
                if learning and (delayed_reward != 0):
                    self.fit(delayed_reward, discount_factor)

            print('>>>learners.ReinforcementLearner.run 에포크 종료 후 학습')
            if learning:
                self.fit(self.agent.profitloss, discount_factor, full=True)

            print('>>>learners.ReinforcementLearner.run 에포크 관련 정보 로그 기록')
            ...
            print('>>>learners.ReinforcementLearner.run 에포크 관련 정보 가시화')
            self.visualize(epoch_str, num_epoches, epsilon)

            print('>>>learners.ReinforcementLearner.run 학습 관련 정보 갱신')
            max_portfolio_value = max(max_portfolio_value, self.agent.portfolio_value)
            if self.agent.portfolio_value > self.agent.initial_balance:
                epoch_win_cnt += 1

        print('>>>learners.ReinforcementLearner.run 종료 시간')
        time_end = time.time()
        elapsed_time = time_end - time_start

        print('>>>learners.ReinforcementLearner.run 학습 관련 정보 로그 기록')
        with self.lock:
    ...
    def save_models(self):
        print('>>>learners.ReinforcementLearner.save_models')
...
class DQNLearner(ReinforcementLearner):
    def __init__(self, *args, value_network_path=None, **kwargs):
        print('>>>learners.DQNLearner.__init__')
    ...
    def get_batch(self, batch_size, delayed_reward, discount_factor):
        print('>>>learners.DQNLearner.get_batch')
...
class PolicyGradientLearner(ReinforcementLearner):
    def __init__(self, *args, policy_network_path=None, **kwargs):
        print('>>>learners.PolicyGradientLearner.__init__')
    ...
    def get_batch(self, batch_size, delayed_reward, discount_factor):
        ...
        print('>>>learners.PolicyGradientLearner.get_batch')
...
class ActorCriticLearner(ReinforcementLearner):
    def __init__(self, *args, shared_network=None,
        ...
        print('>>>learners.ActorCriticLearner.__init__')
    ...
    def get_batch(self, batch_size, delayed_reward, discount_factor):
        ...
        print('>>>learners.ActorCriticLearner.get_batch')
...
class A2CLearner(ActorCriticLearner):
    def __init__(self, *args, **kwargs):
        print('>>>learners.A2CLearner.__init__')
        super().__init__(*args, **kwargs)
    ...
    def get_batch(self, batch_size, delayed_reward, discount_factor):
        ...
        print('>>>learners.A2CLearner.get_batch')
...
class A3CLearner(ReinforcementLearner):
    def __init__(self, *args, list_stock_code=None,
                 ...
                 **kwargs):
        print('>>>learners.A3CLearner.__init__')
